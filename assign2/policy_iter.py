...
def compute_vpi(pi, mdp, gamma):
    ...
    # linear equations parameters A, b such that A * x = b
    A = np.zeros(mdp.nS * mdp.nS).reshape(mdp.nS, mdp.nS) 
    b = np.zeros(mdp.nS)
    
    for s in range(mdp.nS):
        A[s][s] = 1.0 # coefficient of V^pi(s)
        for (prob, s_tag, reward) in mdp.P[s][pi[s]]:
            b[s] += prob * reward
            # moving V^pi(s_tag) to the left side of the equation
            A[s][s_tag] -= prob * gamma
...
vpi = compute_vpi(np.remainder(np.arange(16), mdp.nA), mdp, gamma=GAMMA)
# print("Policy Value: ", vpi)
    ...
    for s in range(mdp.nS):
        for a in range(mdp.nA):
            for (prob, s_tag, reward) in mdp.P[s][a]:
                Qpi[s][a] += prob * (reward + gamma * vpi[s_tag])

    return Qpi

qpi = compute_qpi(vpi, mdp, gamma=0.95)
